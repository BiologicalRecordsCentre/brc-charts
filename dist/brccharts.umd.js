(function (global, factory) {
  ...
  var month2day = [1, 32, 61, 92, 122, 153, 183, 214, 245, 275, 306, 336, 366];
  ...
  function filterYsDomain(monthScaleRange) {
    var minMonth = monthScaleRange[0];
    var maxMonth = monthScaleRange[1];
    return ysDomain.filter(function (m, i) {
      return i + 1 >= minMonth && i + 1 <= maxMonth;
    });
  }
  ...
  function filterMonth2day(monthScaleRange, noAdjust) {
    var minMonth = monthScaleRange[0];
    var maxMonth = monthScaleRange[1];
    var fMonth2day = month2day.filter(function (d, i) {
    ...
    if (noAdjust) {
      ...
      return fMonth2day.map(function (d) {
        return d - month2day[minMonth - 1] + 1;
      });
    }
  }

  function getDayRange(monthScaleRange) {
    var filteredMonth2day = filterMonth2day(monthScaleRange);
  ...
  function xAxisMonthNoText(width, monthScaleRange) {
    var filteredYsDomain = filterYsDomain(monthScaleRange);
    var filteredMonth2day = filterMonth2day(monthScaleRange);
    var dayRange = getDayRange(monthScaleRange);
    var ysRange = filteredMonth2day.map(function (d) {
      return (d - 1) / dayRange * width;
    });
    var xScaleTime = d3.scaleOrdinal().domain(filteredYsDomain).range(ysRange);
    var xAxis = d3.axisBottom().scale(xScaleTime);
    xAxis.ticks(filteredYsDomain).tickSize(width >= 200 ? 13 : 5, 0).tickFormat(function () {
      return '';
    });
    return xAxis;
  }
  function xAxisMonthText(width, ticks, fontSize, font, monthScaleRange) {
    var filteredYsDomain = filterYsDomain(monthScaleRange);
    ...
    for (var i = 0; i < filteredMonth2day.length - 1; i++) {
    ...
    var maxFullMonth = getMaxTextWidth(filteredYsDomain);
    var maxMedMonth = getMaxTextWidth(filteredYsDomain.map(function (m) {
      return m.substr(0, 3);
    }));
    var maxMinMonth = getMaxTextWidth(filteredYsDomain.map(function (m) {
      return m.substr(0, 1);
    }));
    svg.remove();
    var monthNumber = monthScaleRange[1] - monthScaleRange[0] + 1;

    if (ticks) {
      xAxis.ticks(filteredYsDomain).tickSize(0).tickFormat(function (month) {
        if (month === '') {
          return '';
  ...
  function temporalScale(chartStyle, periodType, minPeriod, maxPeriod, xPadding, monthScaleRange, width) {
    ...
    if (periodType === 'month' || periodType === 'week' || periodType === 'day') {
      ...
      scaleFn = function scaleFn(v) {
        return scaleD3(periodToDay(v, periodType, chartStyle));
      };
      ...
      bandwidthFn = function bandwidthFn() {
        return 0;
      };
  ...
  *  </ul>
  ...
  function density() {
    ...
    var updateChart = makeChart(xMin, xMax, data, xlines, ylines, selector, elid, width, height, margin, expand, axisLeft, axisRight, axisTop, axisBottom, axisLeftLabel, axisBottomLabel, axisLabelFontSize, duration, styles, scaleHeight);
    return {
  ...
  function makeChart(xMin, xMax, data, xlines, ylines, selector, elid, width, height, margin, expand, axisLeft, axisRight, axisTop, axisBottom, axisLeftLabel, axisBottomLabel, axisLabelFontSize, duration, styles, scaleHeight) {
    ...
    var updateChart = makeUpdateChart(svgDensity, width, height, tAxis, bAxis, lAxis, rAxis, axisBottom, duration, gChart1, gChart2, styles); // Update the chart with current data
  ...
  function makeUpdateChart(svg, width, height, tAxis, bAxis, lAxis, rAxis, axisBottom, duration, gChart1, gChart2, styles) {
    return function (data, xMin, xMax, xlines, ylines, scaleHeight) {
      ...
      pTrans = [].concat(_toConsumableArray(pTrans), _toConsumableArray(d3Line(gChart1, ylines, 'ylines', t))); // Add path to xlines and generate

      xlines.forEach(function (l) {
        l.path = linePath([[l.x, 0], [l.x, maxDensity * 1.02]]);
      });
      pTrans = [].concat(_toConsumableArray(pTrans), _toConsumableArray(d3Line(gChart1, xlines, 'xlines', t)));
      return Promise.allSettled(pTrans);
    };
  }
  ...
  function bar() {
    ...
    var updateChart = makeChart$1(data, labelPosition, selector, elid, width, height, padding, barHeightOnZero, margin, expand, axisLeft, axisRight, axisTop, axisBottom, axisLeftLabel, axisLabelFontSize, duration, tooltip);
    return {
      updateChart: updateChart,
      saveImage: function saveImage(asSvg, filename, info) {
        return saveChartImage(d3.select("#".concat(elid)), expand, asSvg, filename, null, info);
  ...
  function makeChart$1(data, labelPosition, selector, elid, width, height, padding, barHeightOnZero, margin, expand, axisLeft, axisRight, axisTop, axisBottom, axisLeftLabel, axisLabelFontSize, duration, tooltip) {
    var svgWidth = width + margin.left + margin.right;
    var svgHeight = height + margin.top + margin.bottom; // Append the chart svg
    ...
    var updateChart = makeUpdateChart$1(labelPosition, svgBar, width, height, padding, barHeightOnZero, tAxis, bAxis, lAxis, rAxis, axisBottom, duration, gChart, tooltip); // Update the chart with current data
  ...
  function makeUpdateChart$1(labelPosition, svg, width, height, padding, barHeightOnZero, tAxis, bAxis, lAxis, rAxis, axisBottom, duration, gChart, tooltip) {
  ...
  * <li> <b>errorBarLower</b> - optional name of a numeric property in the data which indicates the lower value
  ...
  function highlightItem$3(id, highlight, svgChart) {
    ...
    if (highlight) {
      ...
      svgChart.selectAll(".temporal-".concat(id)).classed('highlight', true);
    }
  }
  ...
  function generateLines(dataFilteredAll, metricsPlus, gTemporal, t, xScale, yScale, height, pTrans, yminY, periods, minPeriodTrans, maxPeriodTrans, lineInterpolator, missingValues, svgChart, interactivity, chartStyle, composition) {
    ...
    var lineValues = function lineValues(points, iPart) {
      ...
      if (lineInterpolator) {
        ...
        // because resulting number of points in path is not constant.
        d3LineGen.curve(d3__namespace[lineInterpolator]);
      }
    ...
    gLinesBands.selectAll(".temporal-band").data(chartBands, function (d) {
      ...
      return enter.append("path").attr("class", function (d) {
        ...
        return d.fill;
      }).attr("stroke", "none") //.attr("d", d => d.bandPathEnter),
      .attr("d", function (d) {
        return d.bandPath;
      });
    }, function (update) {
  ...
  function generatePointsAndErrors(dataFilteredAll, metricsPlus, gTemporal, t, xScale, yScale, height, pTrans, chartStyle, svgChart, interactivity, composition) {
    ...
    gPoints.selectAll('.temporal-point').data(chartPoints, function (d) {
      ...
      return exit.call(function (exit) {
        return transPromise(exit.transition(t).style("opacity", 0) //.attr('cy', height)
        .remove(), pTrans);
      });
    }) // The selection returned by the join function is the merged
    // enter and update selections
  ...
  function generateSupVerticals(verticals, gTemporal, t, xScale, height, pTrans) {
    ...
    var chartVerticalLines = verticals.filter(function (d) {
      return !d.width;
    }).map(function (d, i) {
      ...
      var x = xScale.d3(d.start);
      return {
        path: "M ".concat(x, " 0 L ").concat(x, " ").concat(height),
        colour: d.colour,
        index: i
      };
    }); // Construct data structures for vertical lines and bars

    var chartVerticalBands = verticals.filter(function (d) {
      return d.width;
    }).map(function (d, i) {
      // Use the D3 scale function (not v) because in charts of type periodType
      // week or month, the start and width values are in *days* not periods
    ...
    gTemporal.selectAll('.temporal-vertical-line-sup').data(chartVerticalLines, function (d) {
      return "vertical-line-sup-".concat(d.index);
    }).join(function (enter) {
      return enter.append('path').attr("class", "temporal-vertical-line-sup").attr("d", function (d) {
        return d.path;
      }).style('stroke', function (d) {
        return d.colour;
      }).style('stroke-width', 1).style('opacity', 0);
    }, function (update) {
      return update;
    }, function (exit) {
      return exit.call(function (exit) {
    ...
    gTemporal.selectAll('.temporal-vertical-band-sup').data(chartVerticalBands, function (d) {
      return "vertical-band-sup-".concat(d.index);
    }).join(function (enter) {
      return enter.append("rect").attr("class", "temporal-vertical-band-sup").attr('width', function (d) {
        return d.width;
      }).attr('height', height).attr('fill', function (d) {
        return d.colour;
      }).attr('opacity', 0).attr('y', 0).attr('x', function (d) {
        return d.x;
      });
    }, function (update) {
      return update;
  ...
  function temporal() {
    var _ref = arguments.length > 0 && arguments[0] !== undefined ? arguments[0] : {},
      ...
      bottom: 20
        ...
        monthScaleRange = _ref$monthScaleRange === void 0 ? [1, 12] : _ref$monthScaleRange,
        _ref$lineInterpolator = _ref.lineInterpolator,
        lineInterpolator = _ref$lineInterpolator === void 0 ? 'curveLinear' : _ref$lineInterpolator,
